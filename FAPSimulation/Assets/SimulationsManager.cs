        ...
        while (true)
        {
            ...
            for (int i = 0; i < productionTasks.Count; i++)
            {
                ...
                if (task["taskStatus"].ToString() == "active")
                {
                    ...
                    {
                        if (ressources[j].state == RessourceManager.RessourceState.Idle && ressources[j].RessourceID == task["ressourceID"].ToString())
                        {
                            ressources[j].state = RessourceManager.RessourceState.Processing;
                            Debug.Log("Ressource Nr " + j + " is processing Task Nr " + i + " now.");
